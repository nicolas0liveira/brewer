...
Brewer.Venda = (function(){
	...
	function onTabelaItensAtualizada(evento, valor){
		this.valorTotalItens =  valor == null ? 0 : valor;
	}
	
	function onValorFreteAlterado(evento){
		this.valorFrete = Brewer.removerFormatacaoMoeda($(evento.target).val());
	}

	function onValorDescontoAlterado(evento){
		this.valorDesconto = Brewer.removerFormatacaoMoeda($(evento.target).val());
	}
